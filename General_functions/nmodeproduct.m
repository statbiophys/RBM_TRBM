function B = nmodeproduct(A,M,n, no_final_shiftdim)
...
% Modified by Christophe Gardella 2017/03: shift does not sequeeze singleton dimensions any more
...
% Ash = shiftdim(A,n-1);
[Ash, dim_l] = shiftdim_nosqueeze(A,n-1);
...
if (nargin<4 || ~no_final_shiftdim)
    B = reshape(B,dimvecB);
end
% shift the dimensions back! so that only dimension n has changed from I_n to J
% B = shiftdim(B,length(dimvecB)-n+1);
if (nargin<4 || ~no_final_shiftdim) && (n ~= 1)
    B = shiftdim_nosqueeze(B,length(dimvecB)-n+1, dimvecB); % Chris: help not squeezing
end
% and we re done!